...
// Package filters implements an bitcoiin2g filtering system for block,
...
type subscription struct {
	...
	typ       Type
	created   time.Time
	logsCrit  bitcoiin2g.FilterQuery
	logs      chan []*types.Log
...
func (es *EventSystem) SubscribeLogs(crit bitcoiin2g.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit bitcoiin2g.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit bitcoiin2g.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit bitcoiin2g.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
