...
const (
	...
	URN_WANBitcoiinnetLinkConfig_1 = "urn:schemas-upnp-org:service:WANBitcoiinnetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Bitcoiinnet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Bitcoiinnet
//
...
// WANBitcoiinnetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANBitcoiinnetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANBitcoiinnetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANBitcoiinnetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANBitcoiinnetLinkConfig1Clients() (clients []*WANBitcoiinnetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANBitcoiinnetLinkConfig_1); err != nil {
		return
	}
	clients = newWANBitcoiinnetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANBitcoiinnetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANBitcoiinnetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANBitcoiinnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANBitcoiinnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANBitcoiinnetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANBitcoiinnetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANBitcoiinnetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANBitcoiinnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANBitcoiinnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANBitcoiinnetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANBitcoiinnetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANBitcoiinnetLinkConfig1 {
	clients := make([]*WANBitcoiinnetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANBitcoiinnetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewBitcoiinnetLinkStatus: allowed values: Up, Down
func (client *WANBitcoiinnetLinkConfig1) GetBitcoiinnetLinkStatus() (NewBitcoiinnetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewBitcoiinnetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANBitcoiinnetLinkConfig_1, "GetBitcoiinnetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewBitcoiinnetLinkStatus, err = soap.UnmarshalString(response.NewBitcoiinnetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
