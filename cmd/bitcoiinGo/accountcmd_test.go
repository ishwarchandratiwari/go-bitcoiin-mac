...
func TestAccountListEmpty(t *testing.T) {
	bitcoiinGo := runBitcoiinGo(t, "account", "list")
	bitcoiinGo.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	bitcoiinGo := runBitcoiinGo(t, "account", "list", "--datadir", datadir)
	defer bitcoiinGo.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	bitcoiinGo := runBitcoiinGo(t, "account", "new", "--lightkdf")
	defer bitcoiinGo.ExpectExit()
	bitcoiinGo.Expect(`
...
`)
	bitcoiinGo.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	bitcoiinGo := runBitcoiinGo(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	bitcoiinGo := runBitcoiinGo(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer bitcoiinGo.ExpectExit()
	bitcoiinGo.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(bitcoiinGo.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(bitcoiinGo.StderrText(), m) {
...
// https://github.com/bitcoiinBT2/go-bitcoiin/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	bitcoiinGo.SetTemplateFunc("keypath", func(file string) string {
