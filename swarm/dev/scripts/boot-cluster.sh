...
#
# The cluster contains a bootnode, a bitcoiinGo node and multiple swarm nodes, with
...
# static bitcoiinGo configuration
...
# network namespaces tobitcoiinGoer
create_network() {
...
# start_bootnode starts a bootnode which is used to bootstrap the bitcoiinGo and
# swarm nodes
start_bootnode() {
...
# start_bitcoiinGo_node starts a bitcoiinGo node with --datadir pointing at <base-dir>/bitcoiinGo
# and a single, unlocked account with password "bitcoiinGo"
start_bitcoiinGo_node() {
  local dir="${base_dir}/bitcoiinGo"
  mkdir -p "${dir}"

  local password="bitcoiinGo"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating bitcoiinGo account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get bitcoiinGo account address"
  fi
  ...
  start_node "bitcoiinGo" "${GETH_IP}" "$(which bitcoiinGo)" ${args[@]}
}
...
create_account() {
  ...
  bitcoiinGo --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
