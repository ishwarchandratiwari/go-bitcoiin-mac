...
#
# The cluster contains a bootnode, a bitcoiin node and multiple swarm nodes, with
...
# network namespaces tobitcoiiner
create_network() {
...
# start_bootnode starts a bootnode which is used to bootstrap the bitcoiin and
# swarm nodes
start_bootnode() {
...
# start_bitcoiin_node starts a bitcoiin node with --datadir pointing at <base-dir>/bitcoiin
# and a single, unlocked account with password "bitcoiin"
start_bitcoiin_node() {
  local dir="${base_dir}/bitcoiin"
  mkdir -p "${dir}"

  local password="bitcoiin"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating bitcoiin account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get bitcoiin account address"
  fi
  ...
  start_node "bitcoiin" "${GETH_IP}" "$(which bitcoiin)" ${args[@]}
}
...
create_account() {
  ...
  bitcoiin --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
